...
1. ✅ Istio debug logs analysis
2. ✅ TCP packet capture (tcpdump) at POD level
3. ✅ Node.js Event Loop monitoring (Dynatrace APM)
4. ✅ In-container process inspection
5. ✅ Idle timeout configuration testing
...
- **Downstream Connection** (Client → Envoy): ConnectionId `10487`
- **Upstream Connection** (Envoy → Application): ConnectionId `161542`
...
### Step 4: ⚠️ Connection Terminated by Application (03:19:42.152160Z)
...
1. ✅ Health check request received by Envoy sidecar
2. ✅ Envoy reused an existing connection (ConnectionId: 161542) from the connection pool
3. ✅ Request successfully transmitted to the application at 100.65.14.224:8080
4. ⚠️ After **~1.956 seconds**, the application closed the TCP connection **without sending any HTTP response**
5. 🔴 Envoy detected the connection termination with 1 pending request
6. 🔴 Envoy generated a **503 Service Unavailable** error as a protective response
...
|----------------|-------------------|
| Application sends HTTP response | ❌ No response sent |
| Application closes connection after response | ❌ Connection closed before response |
| Envoy forwards response to client | ❌ Envoy generates 503 error |
| No error logged | ❌ `Request reset. Reason 7` logged |
| Status code from application | ❌ Envoy-generated 503 |
...
### 1️⃣ TCP Packet Capture (tcpdump) - POD Level
...
**TCP Flag Sequence**:
```
Envoy → Application:  PSH, ACK  (HTTP request sent)
Application → Envoy:  ACK       (Request acknowledged)
Application → Envoy:  RST, ACK  (Connection terminated later)
```
...
### 2️⃣ In-Container Process Inspection
...
### 3️⃣ Idle Timeout Configuration Testing
...
**Result**:
❌ **503 errors continued to occur after applying the timeout configuration**
...
### 4️⃣ Application Performance Monitoring (Dynatrace)
...
|---------|---------|----------------|--------|
| ✅ **Normal** | High tick freq, low latency, balanced utilization | Event loop healthy; issue in specific endpoint or external dependency | Focus on endpoint logic, zombie processes |
| ⚠️ **Degraded** | Low tick freq, high latency, high utilization | Event loop blocked, synchronous operations | Investigate blocking code, CPU-bound tasks |
